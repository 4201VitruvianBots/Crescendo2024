...
public class SysIdArmUtils {
    ...
    public static SysIdRoutine createArmRoutines(Arm arm) {
        return new SysIdRoutine(
            new SysIdRoutine.Config(Volts.of(0.2).per(Units.Seconds.of(1)),
            Volts.of(1.5),
            null
            ),
            new Mechanism(
                ...
                log -> {
                  log.motor("armMotor")
                    .voltage(m_appliedVoltage.mut_replace(
                        arm.getInputVoltage(), Volts)
                    )
                    .angularPosition(m_angle.mut_replace(
                        arm.getCurrentRotation(), Rotations)
                    )
                    .angularVelocity(m_velocity.mut_replace(
                        arm.getRotationalVelocity(), RotationsPerSecond)
                    );
                },
                arm));
    }
