...
public class AutoScore extends Command {
  ...
  public void execute() {
    ...
    if ((m_shooter.getRpmMaster() >= m_RPMOutput - allowableError)
      ...
      if (m_reversetimer.hasElapsed(0.25)) {
        m_ampShooter.setPercentOutput(m_AmpPercentOutput);
        m_intake.setSpeed(m_FrontIntakePercentOutput, m_BackIntakeAmpPercentOutput);
      }

      if (timerStart && m_timer.hasElapsed(m_timeToShoot)) {
        isFinished();
        // withTimeout
      }
      ...
      if ((m_shooter.getRpmMaster() < 300) || (m_shooter.getRpmFollower() < 300)) {
        ...
        m_ampShooter.setPercentOutput(-m_AmpPercentOutput);
        m_intake.setSpeed(-m_FrontIntakePercentOutput, -m_BackIntakeAmpPercentOutput);
        if (m_timer.hasElapsed(m_withTimeout + m_timeToShoot)) {
          isFinished();
          // withTimeout
        }
        ...
        if (timerStart && m_timer.hasElapsed(m_withTimeout + m_timeToShoot)) {
          isFinished();
          // withTimeout
        }
      }
